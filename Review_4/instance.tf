...
resource "aws_key_pair" "projekt_htw_key_pair" {
...
resource "aws_instance" "prometheus_instance" {
  ...
  key_name               = "${aws_key_pair.projekt_htw_key_pair.id}"
...
# Install docker in the ubuntu
  ...
  provisioner "local-exec" {
    command = "echo '${tls_private_key.sskeygen_execution.private_key_pem}' >> ${aws_key_pair.projekt_htw_key_pair.id}.pem ; chmod 400 ${aws_key_pair.projekt_htw_key_pair.id}.pem"
  }
